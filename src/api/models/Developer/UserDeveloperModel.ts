...
export default class UserDeveloperModel {
  ...
  static async findState(nome: string): Promise<Estado | null> {
  ...
  static async findCity(nome: string): Promise<Cidade | null> {
  ...
  static async deleteStacks(id_usuario: number) {
  ...
  static async deleteSkills(id_usuario: number) {
    return await prisma.usuarioStack.deleteMany({
      where: {
        idUsuario: id_usuario,
      },
