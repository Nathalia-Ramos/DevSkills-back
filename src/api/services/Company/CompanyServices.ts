...
export default class CompanyService {
  static async createCompany(user: CompanyUser) {
    ...
    if (userExist == null) {
      if (
        ...
        user.email,
        user.senha,
        user.confirmar_senha)
      ) {
        if (user.cnpj.length > 14 || user.cnpj.length < 14)
            ...
            return { error: "As senhas não combinam" };
          {
              ...
              return { error: "Senha inválida" };
            {
              ...
              try {
                ...
                try {
                  const Address = await UserCompanyModel.createAdress(
                    CompanyAdress
                  );
                  ...
                  try {
                    const newCompany = await UserCompanyModel.create(
                      CompanyUser
                    );
  ...
  static async listTestCompany(
    tokenValidate: number | ErrorReturn,
    filters: filter
  ) {
    if (typeof tokenValidate === "number") {
      const result = await UserCompanyModel.searchTestCompany(
        tokenValidate,
        filters
      );
