...
export default class DeveloperService {
  static async create(userInfo: RegisterDeveloperData) {
    ...
    if (userExist == null) {
      ...
      if (
        (userInfo.nome, userInfo.email, userInfo.cpf, userInfo.data_nascimento)
      ) {
              ...
              try {
                ...
                try {
                  userInfo.ids_habilidades.forEach((skillId) => {
                    DeveloperModel.relateSkills({
                      id_usuario: developerID,
                      id_habilidade: skillId,
                    });
                  });
                } catch (error) {
                  return {
    ...
    if (Object.keys(devProfile).length > 0) {
      if ("id" in tokenValidate) {
        if (tokenValidate.type === "DEVELOPER") {
          if (devProfile.email) {
            const emailExist = await DeveloperModel.findBy(
              "email",
              devProfile.email
            );
          ...
          if (!devUpdated) {
            return {
              error:
                "Não foi possível atualizar os dados base de usuario. (Nome, email, biografia, foto de perfil, redes sociais ou permissao de email.).",
              statusCode: 500,
            };
          }
          ...
          if (devProfile.id_usuario_telefone) {
            ...
            if (
              ...
              if (!newPhone) {
                return {
                  error:
                    "Não foi possivel cadastrar o novo telefone do usuario.",
                  statusCode: 500,
                };
              }
            }
          ...
          if (devProfile.id_usuario_endereco) {
            if (
              ...
              const cityExist = await UserDeveloperModel.findCity(
                devProfile.cidade
              );
              const stateExist = await UserDeveloperModel.findState(
                devProfile.estado
              );
          ...
          if (devProfile.ids_habilidades) {
            ...
            if (deleteSkills) {
              devProfile.ids_habilidades.forEach((skillID) => {
                UserDeveloperModel.relateSkills({
                  id_usuario: userID,
                  id_habilidade: skillID,
                });
              });
          ...
          if (devProfile.ids_stacks) {
            ...
            if (deleteStacks) {
              devProfile.ids_stacks.forEach((stackID) => {
                UserDeveloperModel.relateStacks({
                  id_usuario: userID,
                  id_stack: stackID,
                });
              });
